...
export default class ReportDetailPage {
  ...
  renderSaveButton() {
    ...
    document.getElementById('report-detail-save').addEventListener('click', async () => {
      ...
      const report = await this.#presenter.getReportData(); // pastikan ini mengembalikan data laporan lengkap

      const bookmarks = JSON.parse(localStorage.getItem('bookmarks') || '[]');

      // Cegah duplikasi bookmark berdasarkan ID
      const exists = bookmarks.find(item => item.id === report.id);
      if (!exists) {
        bookmarks.push(report);
        localStorage.setItem('bookmarks', JSON.stringify(bookmarks));
        alert('Laporan berhasil disimpan!');
      } else {
        alert('Laporan sudah ada di bookmark!');
      }
    });
  }
