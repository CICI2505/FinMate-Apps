...
export default class HomePage extends HTMLElement {
  ...
  async connectedCallback() {
    ...
    // Ganti "bookmark" → ke halaman daftar laporan `#/`
    const bookmarkLink = this.shadowRoot.querySelector('a[href="#/bookmark"]');
    if (bookmarkLink) {
      bookmarkLink.addEventListener('click', async (e) => {
        e.preventDefault();

        if (document.startViewTransition) {
          document.documentElement.classList.add('report-transition');
          await document.startViewTransition(() => {
            window.location.hash = '#/';
          }).finished;
          document.documentElement.classList.remove('report-transition');
        } else {
          window.location.hash = '#/';
        }
      });
